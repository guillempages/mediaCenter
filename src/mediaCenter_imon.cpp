...
void usage(string progName) {
    cerr << "Usage:" << endl << "  " << progName << " [-s server] [-p port]" << endl;
}
...
string formatTime(int current, int total, bool paused = false) {
    ...
    if (!paused || time(0) % 2) {
        if (total >= 3600) {
            sstream << setw(3) << setfill(' ') << (current / 3600) % 100 << ":" << setw(2)
                    << setfill('0');
        } else {
            ...
            while (!end) {
                ...
                if (type == "DVD") {
                    ...
                    if (ask(sock, &remote, "Get TotalTime", totalTime) <= 0)
                        break;
                    sstream << setfill(' ') << setw(3) << track << " " << setw(3) << setfill(' ')
                            << chapter;
                    ...
                    sstream << setfill(' ') << setw(digits) << track << "/" << setw(totalDigits)
                            << setfill('0');
