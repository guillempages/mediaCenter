...
PluginConf::PluginConf(const string &_type, const string &_path, const string &_config) :
        class_(""), type(_type), path(_path), configFile(_config), port(10010) {
}

RemotePluginConf::RemotePluginConf(const string &_type, const string &_path, const string &_name,
        const string & _server) :
        PluginConf(_type, _path), name(_name), server(_server) {
    class_ = "Remote";
}

MonitorPluginConf::MonitorPluginConf(const string &_type, const string & _path,
        const string & _device, const string & _mountPoint, const string & _server) :
        PluginConf(_type, _path), device(_device), mountPoint(_mountPoint), server(_server) {
    class_ = "Monitor";
}

OutputPluginConf::OutputPluginConf(const string &_type, const string & _path, const string & _file) :
        PluginConf(_type, _path), file(_file) {
    class_ = "Output";
    port = 10011;
}

DisplayPluginConf::DisplayPluginConf(const string &_type, const string & _path,
        const string & _server) :
        PluginConf(_type, _path), server(_server) {
    class_ = "Display";
    port = 10011;
}

MenuPluginConf::MenuPluginConf(const string &_type, const string & _path, const string & _file,
        const string & _server, int _remotePort) :
        OutputPluginConf(_type, _path, _file), server(_server), remotePort(_remotePort) {
    class_ = "Menu";
    port = 10011;
}

RecordPluginConf::RecordPluginConf(const string & _type, const string & _path, const string & _file,
        const string & _format) :
        PluginConf(_type, _path), file(_file), format(_format) {
    class_ = "Record";
    port = 10011;
}
}
...
std::ostream & operator<<(std::ostream & ostr, const Config::PluginConf& plugin) {
    ostr << "------------------------------------------------" << endl << "Class: " << plugin.class_
            << endl << "Type: " << plugin.type << endl << "Path: " << plugin.path << endl
            << "Config File: " << plugin.configFile << endl << "Port: " << plugin.port << endl;
    return ostr;
}

std::ostream & operator<<(std::ostream & ostr, const Config::RemotePluginConf& plugin) {
    ostr << (const Config::PluginConf&) plugin << "Name: " << plugin.name << endl << "Server: "
            << plugin.server << endl;
    return ostr;
}

std::ostream & operator<<(std::ostream & ostr, const Config::MonitorPluginConf& plugin) {
    ostr << (const Config::PluginConf&) plugin << "Device: " << plugin.device << endl
            << "MountPoint: " << plugin.mountPoint << endl << "Server: " << plugin.server << endl;
    return ostr;
}

std::ostream & operator<<(std::ostream & ostr, const Config::OutputPluginConf& plugin) {
    ostr << (const Config::PluginConf&) plugin << "File: " << plugin.file << endl;
    return ostr;
}

std::ostream & operator<<(std::ostream & ostr, const Config::DisplayPluginConf& plugin) {
    ostr << (const Config::PluginConf&) plugin << "Server: " << plugin.server << endl;
    return ostr;
}

std::ostream & operator<<(std::ostream & ostr, const Config::MenuPluginConf& plugin) {
    ostr << (const Config::OutputPluginConf&) plugin << "Server: " << plugin.server << endl
            << "Remote Port: " << plugin.remotePort << endl;
    return ostr;
}

std::ostream & operator<<(std::ostream & ostr, const Config::RecordPluginConf& plugin) {
    ostr << (const Config::PluginConf&) plugin << "File: " << plugin.file << endl << "Format: "
            << plugin.format << endl;
    return ostr;
}

std::ostream & operator<<(std::ostream & ostr, const Config::Plugins& plugins) {
    ostr << "Config path: " << plugins.path << endl << plugins.remote << plugins.monitor
            << plugins.display << plugins.tv << plugins.dvb << plugins.dvd << plugins.cd
            << plugins.music << plugins.movie << plugins.movieMenu << plugins.record;
    return ostr;
}
