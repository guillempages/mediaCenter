...
HSM_State::HSM_State(HSM* _hsm, const string &_name) :
        hsm(_hsm), name_(_name) {
}
;
...
void HSM_State::receiveEvent(Event *event, bool silent) {
    ...
    if (!silent) {
        DBG(
                cout << "INFO: State >>" << name() << "<< received event >>" << event->name << "<<"; if (event->msg != "") { cout << " with message: <<" << event->msg << ">>"; } cout << endl;);
    }
...
void HSM::changeState(HSM_State *newState) {
    DBG(
            cout << "Changing from state >>" << state->name() << "<< to >>" << newState->name() << "<<" << endl);
